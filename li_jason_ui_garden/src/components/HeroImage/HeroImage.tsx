...
const HeroContainer = styled.div<StyledHeroProps>`
  ...
  overflow: hidden;
  cursor: ${props => props.$disabled ? 'not-allowed' : props.$hasOnClick ? 'pointer' : 'default'};
  opacity: ${props => props.$disabled ? 0.6 : 1};
  filter: ${props => props.$disabled ? 'grayscale(0.3)' : 'none'};
  transition: all 0.3s ease-in-out;

  &:hover {
    opacity: ${props => props.$disabled ? 0.6 : props.$hasOnClick ? 0.9 : 1};
  }

  @media (max-width: 768px) {
    height: ${props => props.$height ? (parseInt(props.$height) * 0.7) + 'px' : '280px'};
  }

  @media (max-width: 480px) {
    height: ${props => props.$height ? (parseInt(props.$height) * 0.5) + 'px' : '200px'};
  }
`;
...
const ContentContainer = styled.div<{ $textAlign?: 'left' | 'center' | 'right'; $textColor?: string }>`
  position: absolute;
  top: 50%;
  left: 50%;
